...
$SupportedWeChatVersions = @("4.1.11.54", "4.1.12.21")
...
function Test-SupportedWeChatProcess([int]$targetPid) {
    ...
    $versionSupported = @($SupportedWeChatVersions | Where-Object {
        $version -like "$_*"
    }).Count -gt 0
    if (-not $versionSupported) {
        Write-Err "不支持的微信版本: $version（仅允许 $($SupportedWeChatVersions -join ', ')）"
        return $false
    }
