...
void ApplyMistyThemeToAllWindows() {
    ...
    struct ThemeTarget {
        DWORD processId;
        int radius;
    } target{GetCurrentProcessId(), blurRadius};

    // Only touch windows owned by the injected host process. Matching every Qt
    // window on the desktop can affect unrelated applications.
    EnumWindows([](HWND hwnd, LPARAM lParam) -> BOOL {
        const auto* target = reinterpret_cast<const ThemeTarget*>(lParam);
        DWORD processId = 0;
        GetWindowThreadProcessId(hwnd, &processId);
        if (processId != target->processId) return TRUE;
        ...
        if (isWeChatWindow && IsWindowVisible(hwnd)) {
            ...
            if (width > 200 && height > 120) {
                ApplyDwmBlur(hwnd, target->radius);
            }
        }
        return TRUE;
