...
set "WECHAT_PID="
for /f "tokens=2" %%i in ('tasklist /fi "IMAGENAME eq Weixin.exe" /fo list 2^>nul ^| findstr "PID:"') do (
    set "WECHAT_PID=%%i"
    goto :found_pid
)
...
if not defined WECHAT_PATH (
    for /f "tokens=2*" %%a in ('reg query "HKCU\SOFTWARE\Tencent\Weixin" /v InstallPath 2^>nul') do (
        set "WECHAT_PATH=%%b"
    )
)
...
if not defined WECHAT_PATH (
    ...
    if exist "D:\Program Files\Tencent\Weixin\Weixin.exe" (
        set "WECHAT_PATH=D:\Program Files\Tencent\Weixin"
    ) else if exist "%ProgramFiles%\Tencent\Weixin\Weixin.exe" (
        set "WECHAT_PATH=%ProgramFiles%\Tencent\Weixin"
    ) else if exist "%ProgramFiles(x86)%\Tencent\Weixin\Weixin.exe" (
        set "WECHAT_PATH=%ProgramFiles(x86)%\Tencent\Weixin"
    ) else if exist "%LOCALAPPDATA%\Programs\Tencent\Weixin\Weixin.exe" (
        set "WECHAT_PATH=%LOCALAPPDATA%\Programs\Tencent\Weixin"
    )
)
...
tasklist /fi "IMAGENAME eq Weixin.exe" 2>nul | findstr "Weixin" >nul
if %errorLevel% neq 0 (
