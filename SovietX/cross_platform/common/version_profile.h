...
struct WeChatProfile {
    ...
    // Windows Profile 必须绑定到承载目标函数的模块。地址使用该模块的
    // PE 虚拟地址（ImageBase + RVA），运行时才允许加 ASLR slide。
    const char* moduleName;
    uint32_t moduleTimeDateStamp;
    uint32_t moduleImageSize;

    // 安装 Hook 前比对入口机器码，避免版本号相同但二进制已变更时误写内存。
    const uint8_t* hookExpectedBytes;
    size_t hookExpectedBytesSize;
