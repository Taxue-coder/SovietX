 ...
 * 
 * 用法：
 *   re_helper.exe <path_to_Weixin.dll>
 *   re_helper.exe <path_to_Weixin.exe>
 */
...
static bool ParsePE(const char* filePath, PEInfo& info) {
    ...
    for (int i = 0; i < fileHeader.NumberOfSections; i++) {
        ...
        sec.rawOffset = section.PointerToRawData;
        sec.rawSize = section.SizeOfRawData;
        sec.characteristics = section.Characteristics;

        printf("    %-8s VA=0x%08X VS=0x%08X FO=0x%08X RS=0x%08X %s%s%s\n",
               sec.name.c_str(),
               sec.virtualAddress, sec.virtualSize, sec.rawOffset, sec.rawSize,
...
static bool FileOffsetToRva(const PEInfo& info, size_t fileOffset, uintptr_t* rva) {
    for (const auto& section : info.sections) {
        size_t start = section.rawOffset;
        size_t end = start + section.rawSize;
        if (fileOffset >= start && fileOffset < end) {
            *rva = section.virtualAddress + (fileOffset - start);
            return true;
        }
    }
    return false;
...
static void PrintContainingFunction(const std::vector<uint8_t>& data,
                                    const PEInfo& info,
                                    uintptr_t instructionRva) {
    for (const auto& section : info.sections) {
        if (section.name != ".pdata") continue;

        size_t start = section.rawOffset;
        size_t end = std::min(start + static_cast<size_t>(section.rawSize), data.size());
        for (size_t offset = start; offset + 12 <= end; offset += 12) {
            DWORD begin = 0;
            DWORD functionEnd = 0;
            memcpy(&begin, data.data() + offset, sizeof(begin));
            memcpy(&functionEnd, data.data() + offset + 4, sizeof(functionEnd));
            if (begin <= instructionRva && instructionRva < functionEnd) {
                printf("      containing function RVA=0x%08X..0x%08X\n", begin, functionEnd);
                return;
            }
        }
        return;
...
static void PrintRipRelativeXrefs(const std::vector<uint8_t>& data,
                                  const PEInfo& info,
                                  uintptr_t targetRva) {
    int printed = 0;
    for (const auto& section : info.sections) {
        ...
        size_t start = section.rawOffset;
        size_t end = std::min(start + static_cast<size_t>(section.rawSize), data.size());
        for (size_t offset = start; offset + 7 <= end; ++offset) {
            // x64 LEA/MOV reg, [RIP + disp32]. These are the common direct
            // references emitted for constant strings in the target module.
            ...
            memcpy(&displacement, data.data() + offset + 3, sizeof(displacement));
            uintptr_t instructionRva = section.virtualAddress + (offset - start);
            intptr_t referencedRva = static_cast<intptr_t>(instructionRva + 7) + displacement;
            if (referencedRva != static_cast<intptr_t>(targetRva)) continue;

            printf("    XREF instruction RVA=0x%08llX\n",
                   static_cast<unsigned long long>(instructionRva));
            PrintContainingFunction(data, info, instructionRva);
            if (++printed == 20) {
                printf("    ... (showing first 20 direct RIP-relative references)\n");
                return;
            }
        }
    }
    ...
    if (printed == 0) {
        printf("    No direct RIP-relative xrefs found\n");
    }
}

static void PrintPointerSlots(const std::vector<uint8_t>& data,
                              const PEInfo& info,
                              uintptr_t stringRva) {
    const uint64_t targetVa = static_cast<uint64_t>(info.imageBase) + stringRva;
    int printed = 0;
    for (size_t offset = 0; offset + sizeof(targetVa) <= data.size(); ++offset) {
        uint64_t value = 0;
        memcpy(&value, data.data() + offset, sizeof(value));
        if (value != targetVa) continue;

        uintptr_t slotRva = 0;
        if (!FileOffsetToRva(info, offset, &slotRva)) continue;
        printf("    Pointer slot file offset=0x%08zX RVA=0x%08llX\n",
               offset, static_cast<unsigned long long>(slotRva));
        PrintRipRelativeXrefs(data, info, slotRva);
        if (++printed == 20) {
            printf("    ... (showing first 20 pointer slots)\n");
            return;
        }
    }
    if (printed == 0) {
        printf("    No image-base pointer slots found\n");
    }
}

static void SearchSelectedStringXrefs(const std::vector<uint8_t>& data,
                                      const PEInfo& info,
                                      const char* keyword) {
    const size_t keywordLength = strlen(keyword);
    int matches = 0;
    printf("--- Xrefs for \"%s\" ---\n", keyword);
    for (size_t offset = 0; offset + keywordLength <= data.size(); ++offset) {
        if (memcmp(data.data() + offset, keyword, keywordLength) != 0) continue;

        uintptr_t rva = 0;
        if (FileOffsetToRva(info, offset, &rva)) {
            printf("  String file offset=0x%08zX RVA=0x%08llX\n",
                   offset, static_cast<unsigned long long>(rva));
            PrintRipRelativeXrefs(data, info, rva);
            PrintPointerSlots(data, info, rva);
        }
        if (++matches == 5) {
            printf("  ... (showing first 5 string instances)\n");
            break;
        }
    }
    if (matches == 0) printf("  String not found\n");
    printf("\n");
}
    ...
    for (auto& keyword : keywords) {
        ...
        for (size_t i = 0; i + keyword.length() <= fileSize; i++) {
            if (memcmp(data.data() + i, keyword.c_str(), keyword.length()) == 0) {
                ...
                uintptr_t rva = 0;
                if (FileOffsetToRva(info, i, &rva)) {
                    printf("  [FOUND] Offset=0x%08zX RVA=0x%08llX\n",
                           i, static_cast<unsigned long long>(rva));
                } else {
                    printf("  [FOUND] Offset=0x%08zX (not mapped to a section RVA)\n", i);
                }
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        ...
        printf("Examples:\n");
        printf("  %s \"D:\\Program Files\\Tencent\\Weixin\\4.1.11.54\\Weixin.dll\"\n", argv[0]);
        printf("  %s \"D:\\Program Files\\Tencent\\Weixin\\Weixin.exe\"\n", argv[0]);
        return 1;
    }
    ...
    // These strings are confirmed in Weixin 4.1.11.54 and are useful entry
    // points for validating client-specific behavior before a Profile is filled.
    FILE* fp = fopen(filePath, "rb");
    if (fp) {
        fseek(fp, 0, SEEK_END);
        size_t fileSize = ftell(fp);
        ...
        if (fread(data.data(), 1, fileSize, fp) == fileSize) {
            printf("\n=== Direct String Xrefs ===\n\n");
            SearchSelectedStringXrefs(data, info, "revokemsg");
            SearchSelectedStringXrefs(data, info, "_WeChat_Win_User_Identity_Mutex_Name");
        }
        fclose(fp);
    }
