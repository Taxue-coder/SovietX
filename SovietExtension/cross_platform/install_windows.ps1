...
#   .\install_windows.ps1 -Uninstall        # 卸载（从进程移除）
# ============================================================
...
function Get-WeChatProcesses {
    return Get-Process -Name "WeChat" -ErrorAction SilentlyContinue
}
...
function Get-WeChatInstallPath {
    ...
    foreach ($regPath in $regPaths) {
        if (Test-Path $regPath) {
            ...
            if ($installLoc -and (Test-Path (Join-Path $installLoc "WeChat.exe"))) {
                return $installLoc
            }
        }
    }
    ...
    foreach ($path in $commonPaths) {
        if (Test-Path (Join-Path $path "WeChat.exe")) {
            return $path
        }
    }
...
function Start-WeChatAndWait {
    ...
    $exePath = Join-Path $wechatPath "WeChat.exe"
...
function Invoke-Inject([int]$pid) {
    Write-Info "正在注入到 PID $pid..."

    $result = & $InjectorPath $pid 2>&1
...
# 检查管理员权限
if (-not (Test-Admin)) {
    Write-Err "需要以管理员身份运行！"
    Write-Host "请右键 PowerShell → 以管理员身份运行"
    exit 1
}
Write-Ok "管理员权限确认"
